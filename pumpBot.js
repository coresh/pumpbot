...
function buyBinance(info) {
  ...
  let max_digits = getMaxDigitsForSell(info.binanceFormatInfo);
  console.log(`calculated shares ${shares}`);
  shares = convertToCorrectLotSize(shares,info.binanceFormatInfo);
  split_shares = convertToCorrectLotSize(shares*.24,info.binanceFormatInfo);
  console.log(`buying ${shares} of ${info.coin} on binance`);
  const flags = {type: 'MARKET', newOrderRespType: 'FULL'};
  api.binance.marketBuy(info.coin.toUpperCase()+"BTC", shares, flags,function(response) {
    ...
    // });
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.15)).toFixed(max_digits));
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.20)).toFixed(max_digits));
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.25)).toFixed(max_digits));
    api.binance.sell(info.coin.toUpperCase()+"BTC", split_shares.toFixed(2), (info.binancePrice+(info.binancePrice*.30)).toFixed(max_digits));
  });
}
...
function getMaxDigitsForSell(requirement){
	min_price = parseFloat(requirement.minPrice);
	x=Math.log10(parseInt(1/min_price));
	console.log(`MAX DIGITS ${x}`);
